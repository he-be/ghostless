# Unity Development Guide for Ghostless

## 3. The C# Script (`GhostlessReceiver.cs`)

**Updated for Execution Order (v5).**

If specific blendshapes (like Blink) aren't moving, the VRM system is likely overwriting them in the same frame.
This version adds `[DefaultExecutionOrder(20000)]` to ensure our changes are applied **after** VRM has finished its work.
...
[DefaultExecutionOrder(20000)] // Run AFTER VRM (usually ~10000)
...
public class GhostlessReceiver : MonoBehaviour
{
    ...
    public string blinkShapeL = "Fcl_EYE_Close_L"; 
    public string blinkShapeR = "Fcl_EYE_Close_R"; 
    ...
    private int idxBlinkL = -1, idxBlinkR = -1;
    private int idxBrows = -1, idxMouth = -1;
    ...
    void Initialize()
    {
        ...
        if (head && spine && chest) {
            ...
            if (faceMesh != null)
            {
                ...
                if (mesh != null)
                {
                    idxBlinkL = mesh.GetBlendShapeIndex(blinkShapeL);
                    idxBlinkR = mesh.GetBlendShapeIndex(blinkShapeR);
    ...
    void ApplyFaceDirect()
    {
        ...
        if (idxBlinkL != -1) faceMesh.SetBlendShapeWeight(idxBlinkL, tBlink * 100f);
        if (idxBlinkR != -1) faceMesh.SetBlendShapeWeight(idxBlinkR, tBlink * 100f);
